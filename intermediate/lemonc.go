...
func main() {
	...
	flag.BoolVar(&mhflag, "m", false, "Output a makeheaders compatible file.")
...
func translate_code(lemp *lemon, rp *rule) int {
	...
	if len(rp.rhs) == 0 {
		...
		if has_destructor(rp.rhs[0], lemp) {
			buf.Reset()
			buf.WriteString(replaceNumbers("  yy_destructor(yypParser,%d,&yymsp[%d].minor);\n", rp.rhs[0].index, 1-len(rp.rhs)))
	...
	if lhsdirect {
		zLhs = fmt.Sprintf("yymsp[%d].minor.yy%d", 1-len(rp.rhs), rp.lhs.dtnum)
	} else {
		rc = 1
				...
				for i := range rp.rhs {
					if rp.rhsalias[i] != "" && runesStringEqual(substr, rp.rhsalias[i]) {
						if i == 0 && dontUseRhs0 {
							...
							removeLastRune(&buf)
							buf.WriteString(replaceNumbers("yymsp[%d].major", i-len(rp.rhs)+1, 0))
						} else {
							sp := rp.rhs[i]
							var dtnum int
							if sp.typ == MULTITERMINAL {
							...
							buf.WriteString(replaceNumbers("yymsp[%d].minor.yy%d", i-len(rp.rhs)+1, dtnum))
						}
						cp = xp
						used[i] = true
						break
	 ...
	 ** saved LHS value now. */
	if !lhsdirect {
		buf.WriteString(replaceNumbers("  yymsp[%d].minor.yy%d = ", 1-len(rp.rhs), rp.lhs.dtnum))
		buf.WriteString(zLhs)
		buf.WriteString(";\n")
	}
...
func print_stack_union(
	...
	mhflag bool, /* True if generating makeheaders output */
) {
	...
	fmt.Fprintf(out, "#define %sTOKENTYPE %s\n", name, tokentype)
	lineno++
	if mhflag {
		fmt.Fprintf(out, "#endif\n")
		lineno++
	}
	fmt.Fprintf(out, "typedef union {\n")
	lineno++
	fmt.Fprintf(out, "  int yyinit;\n")
	lineno++
...
func ReportTable(lemp *lemon,
	mhflag bool, /* Output in makeheaders format if true */
	...
	if mhflag {
		...
		fmt.Fprintf(out, "#ifndef %s%s\n", prefix, lemp.symbols[1].name)
	}

	for i := 1; i < lemp.nterminal; i++ {
		fmt.Fprintf(out, "#define %s%-30s %2d\n", prefix, lemp.symbols[i].name, i)
		lineno++
	}
	fmt.Fprintf(out, "#endif\n")
	lineno++
	...
	fmt.Fprintf(out, "#define YYCODETYPE %s\n",
		minimum_size_type(0, lemp.nsymbol, &szCodeType))
	lineno++
	fmt.Fprintf(out, "#define YYNOCODE %d\n", lemp.nsymbol)
	lineno++
	fmt.Fprintf(out, "#define YYACTIONTYPE %s\n",
		minimum_size_type(0, lemp.maxAction, &szActionType))
	lineno++
	if lemp.wildcard != nil {
		fmt.Fprintf(out, "#define YYWILDCARD %d\n",
			lemp.wildcard.index)
		lineno++
	}
	print_stack_union(out, lemp, &lineno, mhflag)
	fmt.Fprintf(out, "#ifndef YYSTACKDEPTH\n")
	lineno++
	if lemp.stacksize != "" {
		fmt.Fprintf(out, "#define YYSTACKDEPTH %s\n", lemp.stacksize)
		lineno++
	} else {
		fmt.Fprintf(out, "#define YYSTACKDEPTH 100\n")
		lineno++
	}
	fmt.Fprintf(out, "#endif\n")
	lineno++
	...
	findSuffix := func(s string) string {
		runes := []rune(s)
		i := len(runes)
		for ; i >= 1 && unicode.IsSpace(runes[i-1]); i-- {
		}

		for ; i >= 1 && (isalnum(runes[i-1]) || runes[i-1] == '_'); i-- {
		}
		return string(runes[i:])
	}

	if lemp.arg != "" {
		suffix := findSuffix(lemp.arg)
		fmt.Fprintf(out, "#define %sARG_SDECL %s;\n", name, lemp.arg)
		lineno++
		fmt.Fprintf(out, "#define %sARG_PDECL ,%s\n", name, lemp.arg)
		lineno++
		fmt.Fprintf(out, "#define %sARG_PARAM ,%s\n", name, suffix)
		lineno++
		fmt.Fprintf(out, "#define %sARG_FETCH %s=yypParser->%s;\n", name, lemp.arg, suffix)
		lineno++
		fmt.Fprintf(out, "#define %sARG_STORE yypParser->%s=%s;\n", name, suffix, suffix)
		lineno++
	} else {
		fmt.Fprintf(out, "#define %sARG_SDECL\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sARG_PDECL\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sARG_PARAM\n", name)
		lineno++
	...
	if lemp.ctx != "" {
		suffix := findSuffix(lemp.ctx)
		fmt.Fprintf(out, "#define %sCTX_SDECL %s;\n", name, lemp.ctx)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_PDECL ,%s\n", name, lemp.ctx)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_PARAM ,%s\n", name, suffix)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_FETCH %s=yypParser->%s;\n", name, lemp.ctx, suffix)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_STORE yypParser->%s=%s;\n", name, suffix, suffix)
		lineno++
	} else {
		fmt.Fprintf(out, "#define %sCTX_SDECL\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_PDECL\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_PARAM\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_FETCH\n", name)
		lineno++
		fmt.Fprintf(out, "#define %sCTX_STORE\n", name)
		lineno++
	}
	if mhflag {
		fmt.Fprintf(out, "#endif\n")
	...
	if lemp.errsym != nil && lemp.errsym.useCnt != 0 {
		fmt.Fprintf(out, "#define YYERRORSYMBOL %d\n", lemp.errsym.index)
		lineno++
		fmt.Fprintf(out, "#define YYERRSYMDT yy%d\n", lemp.errsym.dtnum)
		lineno++
	}
	if lemp.has_fallback {
		fmt.Fprintf(out, "#define YYFALLBACK 1\n")
		lineno++
	}
	...
	** been computed */
	fmt.Fprintf(out, "#define YYNSTATE             %d\n", lemp.nxstate)
	lineno++
	fmt.Fprintf(out, "#define YYNRULE              %d\n", lemp.nrule)
	lineno++
	fmt.Fprintf(out, "#define YYNRULE_WITH_ACTION  %d\n", lemp.nruleWithAction)
	lineno++
	fmt.Fprintf(out, "#define YYNTOKEN             %d\n", lemp.nterminal)
	lineno++
	fmt.Fprintf(out, "#define YY_MAX_SHIFT         %d\n", lemp.nxstate-1)
	lineno++
	i := lemp.minShiftReduce
	fmt.Fprintf(out, "#define YY_MIN_SHIFTREDUCE   %d\n", i)
	lineno++
	i += lemp.nrule
	fmt.Fprintf(out, "#define YY_MAX_SHIFTREDUCE   %d\n", i-1)
	lineno++
	fmt.Fprintf(out, "#define YY_ERROR_ACTION      %d\n", lemp.errAction)
	lineno++
	fmt.Fprintf(out, "#define YY_ACCEPT_ACTION     %d\n", lemp.accAction)
	lineno++
	fmt.Fprintf(out, "#define YY_NO_ACTION         %d\n", lemp.noAction)
	lineno++
	fmt.Fprintf(out, "#define YY_MIN_REDUCE        %d\n", lemp.minReduce)
	lineno++
	i = lemp.minReduce + lemp.nrule
	fmt.Fprintf(out, "#define YY_MAX_REDUCE        %d\n", i-1)
	lineno++
	...
	fmt.Fprintf(out, "#define YY_MIN_DSTRCTR       %d\n", mn)
	lineno++
	fmt.Fprintf(out, "#define YY_MAX_DSTRCTR       %d\n", mx)
	lineno++
	...
	**  yy_default[]       Default action for each state.
	...
	fmt.Fprintf(out, "#define YY_ACTTAB_COUNT (%d)\n", n)
	lineno++
	fmt.Fprintf(out, "static const YYACTIONTYPE yy_action[] = {\n")
	lineno++
	for i, j := 0, 0; i < n; i++ {
	...
	fmt.Fprintf(out, "static const YYCODETYPE yy_lookahead[] = {\n")
	lineno++
	i, j := 0, 0
	for i, j = 0, 0; i < n; i++ {
	...
	for i < nLookAhead {
		...
		fmt.Fprintf(out, " %4d,", lemp.nterminal)
		if j == 9 {
			fmt.Fprintf(out, "\n")
			lineno++
			j = 0
	...
	fmt.Fprintf(out, "#define YY_SHIFT_COUNT    (%d)\n", n-1)
	lineno++
	fmt.Fprintf(out, "#define YY_SHIFT_MIN      (%d)\n", mnTknOfst)
	lineno++
	fmt.Fprintf(out, "#define YY_SHIFT_MAX      (%d)\n", mxTknOfst)
	lineno++
	fmt.Fprintf(out, "static const %s yy_shift_ofst[] = {\n",
	...
	fmt.Fprintf(out, "#define YY_REDUCE_COUNT (%d)\n", n-1)
	lineno++
	fmt.Fprintf(out, "#define YY_REDUCE_MIN   (%d)\n", mnNtOfst)
	lineno++
	fmt.Fprintf(out, "#define YY_REDUCE_MAX   (%d)\n", mxNtOfst)
	lineno++
	fmt.Fprintf(out, "static const %s yy_reduce_ofst[] = {\n",
	...
	fmt.Fprintf(out, "static const YYACTIONTYPE yy_default[] = {\n")
	lineno++
	n = lemp.nxstate
	lemp.tablesize += n * szActionType
	for i, j = 0, 0; i < n; i++ {
		...
		if stp.iDfltReduce < 0 {
			fmt.Fprintf(out, " %4d,", lemp.errAction)
		} else {
			fmt.Fprintf(out, " %4d,", stp.iDfltReduce+lemp.minReduce)
		}
		if j == 9 || i == n-1 {
			fmt.Fprintf(out, "\n")
			lineno++
	 ...
	 */
	for i, rp := 0, lemp.rule; rp != nil; rp, i = rp.next, i+1 {
		fmt.Fprintf(out, "  %4d,  /* (%d) ", rp.lhs.index, i)
		rule_print(out, rp)
		fmt.Fprintf(out, " */\n")
		lineno++
	}
	tplt_xfer(lemp.name, in, out, &lineno)
	for i, rp := 0, lemp.rule; rp != nil; rp, i = rp.next, i+1 {
		fmt.Fprintf(out, "  %3d,  /* (%d) ", -len(rp.rhs), i)
		rule_print(out, rp)
		fmt.Fprintf(out, " */\n")
		lineno++
	}
	...
	if i != 0 {
		fmt.Fprintf(out, "        YYMINORTYPE yylhsminor;\n")
		lineno++
	}
	...
	for rp := lemp.rule; rp != nil; rp = rp.next {
		...
		if rp.neverReduce {
			...
			fmt.Fprintf(out, " (OPTIMIZED OUT) */ assert(yyruleno!=%d);\n",
				rp.iRule)
			lineno++
		}
	}
...
func ReportHeader(lemp *lemon) {
	...
	if out != nil {
		for i := 1; i < lemp.nterminal; i++ {
			fmt.Fprintf(out, "#define %s%-30s %3d\n", prefix, lemp.symbols[i].name, i)
		}
		out.Close()
	}
}
...
func (us unsetFlag) Set(value string) error {
	delete(us.sf, value)
	return nil
}
